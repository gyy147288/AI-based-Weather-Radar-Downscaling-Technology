...
if __name__ == "__main__":
    # 定义文件路径（请根据实际情况修改）相控阵
    file_zh_1 = "run_data/0429135859data/layer_5_reflectivity.csv"
    # file_zh_1 = "run_data/0429data/layer_4.3_reflectivity.csv"
    file_cc_1 = "run_data/0430022017data/layer_5_CC.csv"
    file_zh_2 = "run_data/0429135859data/layer_7_reflectivity.csv"
    # file_zh_2 = "run_data/0430022017data/layer_1.45_interpolated.csv"
    file_cc_2 = "run_data/0430022017data/layer_7_CC.csv"
    file_zh_3 = "run_data/0429135859data/layer_4.3_reflectivity.csv"
    # file_zh_3 = "run_data/0429135859data/layer_11_reflectivity.csv"
    file_cc_3 = "run_data/0430022017data/layer_10_CC.csv"

    # 定义文件路径（请根据实际情况修改）sa雷达
    # file_zh_1 = "run_data/sa_data/0430022017dbz/layer_1_reflectivity.csv"
    # file_zh_2 = "run_data/sa_data/0430022017dbz/layer_2_reflectivity.csv"
    # file_zh_3 = "run_data/sa_data/0430022017dbz/layer_3_reflectivity.csv"
    ...
    shifts = calculate_relative_shifts(select_distances, elvs=elvs, reference_elv=2.4)
    ...
    # 加载模型（请修改模型路径）
    # model = load_trained_model("best_models_resnet/best_model3.0.pth")
    model = load_trained_model("best_models_forplot/best_model_0.89dbz.pth")

    predictions = predict(model, dataloader)
    vectorized_round = np.vectorize(safe_round, otypes=[float])
    rounded = vectorized_round(predictions, decimals=2)
    # predictions = np.round(rounded * 100).astype(np.int64)  # 为了输出数据,用于二进制
    ...
    # 将预测值低于 -500 的置为 NaN
    reshaped_predictions = np.where(reshaped_predictions <= 3, np.nan, reshaped_predictions)

    # **填补 NaN 和 TRUE_FILL (-1000)**
    # reference_layer = data[0][1:417,0:3999] / 100.0  # 选定对照层，如 4.3° 层
    reference_layer = data[0][1:362, 0:999] / 100.0  # 选定对照层，如 4.3° 层
    mask_empty = (reshaped_predictions <= 0) | np.isnan(reshaped_predictions)
    reshaped_predictions[mask_empty] = reference_layer[mask_empty]  # 用对照层填补

    # **如果对照层也是空值，保持 NaN**
    reshaped_predictions[reshaped_predictions <= -5] = np.nan
    reshaped_predictions = reshaped_predictions * 100
    # 添加仰角到第一列
    reshaped_predictions = np.column_stack((base_az, reshaped_predictions))
    # pd.DataFrame(reshaped_predictions).to_csv("run_data/result/predictions_new.csv", index=False)
    # pd.DataFrame(reshaped_predictions).to_csv("run_data/result_dbz/0429135859/test_3.0.csv", index=False)
    pd.DataFrame(reshaped_predictions).to_csv("run_data/sa_data/0430022017dbz/layer_0.89_reflectivity.csv", index=False)
